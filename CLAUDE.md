...
- `sync_work_orders` Procrastinate task wired to the sync service
...
- **REST API endpoints** for work orders: paginated list with filters
  and single-WO detail, both with eager-loaded Client/Location/Trade refs
- End-to-end verified: 167 work orders in Supabase, queryable via the API
- 26 tests passing (transformers, SC client, auth, API endpoints)

**Not yet done:**
- `sync_work_order_detail` (notes endpoint) is still a placeholder
- Auth integration (Supabase Auth + JWT verification on the API)
- Recency-filter semantics need reconsideration before production
  (TODO is in `app/services/sync/work_orders.py`)
...
1. **Revisit the recency filter** in
   `app/services/sync/work_orders.py` before production. The current
   "skip if `UpdatedDate < cutoff`" logic theoretically misses long-lived
   in-progress WOs. Likely answer: separate "full sync" mode for backfill
   + periodic reconciliation, or drop the filter entirely and rely on
   upsert idempotency.
2. Build `sync_work_order_detail` to pull notes via
   `/v3/workorders/{id}/notes` and upsert into `wo_notes`. Decide
   trigger: every WO sync vs. only when `notes_count` changes.
3. Wire up Supabase Auth + JWT verification on the FastAPI endpoints
   (use `SUPABASE_JWT_SECRET` from config).
4. Begin the Next.js dashboard scaffold and wire it to the API.
5. Cleanup pass: 10 pre-existing ruff errors in `config.py`,
